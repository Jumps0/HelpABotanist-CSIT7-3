...
df = pd.read_csv('/Users/jocelynemululu/Downloads/Aalborg University/Project/Project code and tables/HelpABotanist-CSIT7-3-main/denmarkgrid.csv')

# Set the flags
require_positive_occurrence = True  # If TRUE, the central square must have at least one instance of positive occurrence.
random_test_split = True  # If TRUE, 20% of data will be selected randomly for testing.
...
if visualize:
    ...
    ax.set_extent([df['longitude'].min() - 0.5, df['longitude'].max() + 1.0 - 0.5,
                   df['latitude'].min() - 0.5, df['latitude'].max() + 0.5],
